...
The default endpoints below mirror the .env.example MY_SVC_PATHS list:
  ...
  GET  /meta    machine-readable service description (register-time probe)
  POST /stream  5-tick chunked demo for server-stream mode

Replace these with your own routes. Keep at least ``/health`` and ``/meta`` —
the gateway uses them to surface your service in ``anet svc health`` /
``anet svc meta`` so it can be introspected by other agents.
"""
        ...
        "endpoints": [
            {
                ...
                "path": "/echo",
                "in": {"msg": "string"},
                "out": {"echo": "object", "caller_did": "string"},
            },
            {
                "method": "POST",
                "path": "/stream",
                "out": "text/plain (newline-delimited JSON)",
            },
        ],
    }
...
async def echo(
    ...
    """Echo the request body back, including the caller's DID."""
    body = await req.body()
    try:
...
async def stream() -> StreamingResponse:
    """Demo of server-stream mode: emit 5 JSON chunks, then end.

    The gateway forwards each newline-terminated chunk back to the caller.
