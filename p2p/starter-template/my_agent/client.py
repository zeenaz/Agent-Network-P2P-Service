...
Usage::
    ...
    python -m my_agent.client --stream         # consume server-stream
...
4. Print the audit row with ``svc.audit(limit=1)`` so you can verify the
   gateway wrote the call into ``svc_call_log``.
"""
...
def main() -> int:
    ...
    with svc:
        ...
        if args.stream:
            print(
                f"\n[client] streaming {svc_name} on "
                f"{peer_id[:18]}… {args.path}"
            )
