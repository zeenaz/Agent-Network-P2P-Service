"""Tiny stdlib HTTP echo backend — no FastAPI / uvicorn dependency.

Listens on ``127.0.0.1:$ECHO_PORT`` (default 7100). Echoes the POST body back
as JSON, exposes ``/health`` for ``anet svc health``, ``/meta`` for
``anet svc meta``.
"""
