...
![资源错配问题图](assets/resource-mismatch-zh.png)
...
> **图片占位 4：AgentReserve 四层概念栈**
> 建议文件：`assets/agentreserve-stack.png`
> 建议内容：自下而上展示流动性池、代理调用与用量计量、信用感知资源配置、任务成本评估；旁边用标签标注当前演示覆盖范围。
...
> **图片占位 5：生态价值图**
> 建议文件：`assets/ecosystem-value.png`
> 建议内容：展示 Key Bank 如何连接任务需求、Agent 执行能力、闲置资源和信用记录，突出“资源利用率提升”和“履约记录复用”。
...
> **图片占位 6：系统架构图**
> 建议文件：`assets/architecture.png`
> 建议内容：建议画成三层：Agent Network 调用层、Key Bank 核心层、Provider 代理层；在 Key Bank 核心层标出 DID、短期授权令牌、请求日志、风险信号。
